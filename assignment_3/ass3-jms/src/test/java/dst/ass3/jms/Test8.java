...
/**
 * This test performs the following tasks: Start one computer, stop it, ASSIGN 1
 * task and RATE it as CHALLENGING. Start computer again and PROCESS it. Test
 * checks if computer that are currently not listening also receive messages,
 ...
 *                                                                                        
 * CP1: Check-Point 1 - Assign Job 1; Shutdown pc1                                         
 * CP2: Check-Point 2 - Start pc1 again; wait till Job 1 has finished                         
...
public class Test8 extends AbstractJMSTest {
    ...
    @Test
    public void test_ComputerListener() {
        sem = new Semaphore(0);
        ...
        log("Shutdown pc1 so it isn't listening anymore!");
        pc1.stop(); // pc1 is offline now => should not listen anymore
        ...
        // start pc1 again => should receive a message and process it
        log("Start pc1 again!");
        pc1.setComputerListener(computerListener);
        pc1.start();
        
        sleep(SHORT_WAIT); // give computer some time to start again.
